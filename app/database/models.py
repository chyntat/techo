...
class Follower(Base):
    ...
    follower_id = Column(UUID, primary_key=True, index=True)
    is_active = Column(Boolean, default=True)
    influencer_id = Column(UUID, ForeignKey("users.user_id", deferrable=True, initially='DEFERRED'), index=True)
    created_at = Column(DateTime, server_default=func.now())
    updated_at = Column(DateTime, server_default=func.now(), onupdate=func.current_timestamp())
    follower = relationship("User", foreign_keys=[follower_id])
    followed = relationship("User", foreign_keys=[influencer_id])


class JournalEntry(Base):
    __tablename__ = "journal_entries"
    journal_id = Column(UUID, primary_key=True, index=True)
    user_id = Column(UUID, ForeignKey("users.user_id", deferrable=True, initially='DEFERRED'), index=True)
    created_at = Column(DateTime, server_default=func.now())
    updated_at = Column(DateTime, server_default=func.now(), onupdate=func.current_timestamp())
    title = Column(String(128), mutable=True)
    content = Column(Text)
    tags = Column(JSONB)
    journal_hash = Column(String(128), index=True)  # for future, to compare hashes for analytics purposes
    media_urls = Column(Text, mutable=True)  # this is a comma separated string of urls
    ...
    location = Column(String(128), nullable=True)
    mentions = Column(user_id, uselist=True, foriegn_keys=[user_id])  # TODO: check if this is correct
    nsfw_score = Column(JSONB)
    profanity_words = Column(Text, mutable=True)
